...
export default function CampaignDetailPage() {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-slate-400">Loading target...</div>
      </div>
    );
  }
  ...
  if (!campaign) {
    return (
      <div className="flex flex-col items-center justify-center h-64 text-center">
        ...
        <p className="text-slate-400 mb-4">Target not found</p>
  ...
  const updateStatus = async (status: CampaignStatus) => {
    try {
      ...
      setMutationError(`Failed to update target status to "${status}".`);
    }
  };
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <h2 className="text-base font-semibold text-red-100">
                  Unable to calculate target plan
                </h2>
                <p className="mt-1 text-sm text-red-200/80">
                  {planError ?? 'Try refreshing the app or updating this target setup.'}
                </p>
              </div>
            </div>
          </CardContent>
            ...
            <CardContent>
              ...
              {deficitMaterials.length === 0 ? (
                <p className="text-sm text-slate-500">
                  No material deficits for this target.
                </p>
              ) : (
                <div className="space-y-2">
  ...
  return (
    <div className="space-y-2">
      ...
      {recommendations.length > 4 && (
        <p className="text-xs text-slate-500">
          {recommendations.length - 4} more talent deficits are included in this target.
        </p>
      )}
    </div>
  );
